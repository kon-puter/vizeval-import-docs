...
Existuje však \codename{MultiClassGroup}, což je skupina složená z více skupin. Ta je použita v kroku spojování skupin (viz. \ref{postup:spojovani-skupin}) a po tomto kroku je spojena na standardní skupinu.
Tato entita je nutná, aby bylo možné opustit import a později se k němu vrátit.

Údaje o importech se nachází v \codename{ImportStatus}. Ten má relace na všechny \codename{ImportedEntity}, aby byl znám původ instance dané entity, to je důležité při zrušení importu (viz. \ref{postup:zruseni-importu}).
...
V našem případě je tato stratgie využita u entit \codename{User}, \codename{ImportedPerson}, \codename{Teacher}, \codename{Student}. 
...
%     \centering
%
%     \resizebox{1\textwidth}{!}{\huge{\input{Figures/user-inharitance.tex}}}
%
%     \caption{UML diagram dědičnosti uživatele}
%     \label{fig:poskytovatele-emailu}
% \end{figure}
...
\begin{sidewaysfigure}
	\centering
  \resizebox{0.95\textwidth}{!}{\huge{\input{Figures/er-diagram.tex}}}
  \caption{ER diagram}
...
Uživatele máme řešené dědičností tak, že třídy \codename{Student} a \codename{Teacher} dědí z abstraktní třídy \codename{ImportedPerson} a ta dědí ze třídy \codename{User}.
...
Po každých $N$ operacích se pak zavolají metody \codename{EntityManager.flush()} a \codename{EntityManager.clear()}.
Dělá se to po inkrementech, aby \codename{UnitOfWork} byl schopen udělat možné optimalizace a aby se zamezilo zbytečnému procházení grafu entit. 
Operace clear pak vyčistí graf, to je důležité aby nedošlo k úniku paměti.

Alternativou k vyčištění cache Doctrine je použít \codename{deatach} na každou instanci entity. Tato možnost byla ve finále zvolena, aby jsme neodstranili ještě potřebné instance entit z cache. 

\codename{ImportedEntity} (viz. listing \ref{postup:imported-entity}) má metodu loadImportHash, která se používá ke vytvoření import hashe.
...
Import hash je využíván třídou \codename{ImportedEntityUpsertProcessor}, která se stará o upsert importovaných entit.
\codename{ImportedEntityUpsertProcessor} načte při upsertu všechny instance upsertované entity.
Potom naplní \codename{ImportedEntityCache}, což je mapa import hashů na id.
Pak se prochází instance entit, které byly právě naimportovány a pokud nejsou v cache tak se uloží pomocí hromadné operace a staré \codename{ImportedEntity} se upraví pomocí metody \codename{updateFromSelf}\footnote{Tato operace nemusí být využita}.
...
To jaký krok bude další a aktuální stav určuje konečný automat \vizref{uvod:konecny-automat}.
...
U definice tohoto konečného automatu je v metadatech stavu uložen \codename{presenter}, který má za úkol zobrazení a zpracování daného stavu. 
...
\unsure{Dej to do spojování skupin}
...
Kolečko znázorňující načítaní se ukazuje, když má na sobě element \codename{turbo-frame} stav \codename{busy}, to typicky nastává když turbo čeká na odpověď od serveru.
...
\begin{figure}
	\centering
\resizebox{!}{0.95\textheight}{\huge{\input{sections/graph.tex}}}
...
Entity se ale nepersistují hned a ukládají se do databáze až v \codename{InitialImportService}.

Importry z XML a sestav implementují interface \codename{ImportDataProvider}. Ten se skládá z getterů vracejících managery.
Managery jsou objekty, co se starají o správu daných entit.
\codename{InitalImportService} potom příjme \codename{ImportDataProvider} a postará se o jeho uložení. 

Různé zdroje dat mohou potřebovat rozdílné způsoby uložení.
\codename{InitialImportService} přijímá kroky prvotního importu, které implementují \codename{InitialImportStepInterface}.
...
Presenter pro první krok (\codename{InitialImportPresenter}) potom najde v DI kontejneru form providery.
...
Import z XML je nezávislý na školním systému. Je však potřeba mít externí program, který XML vytvoří dle daného RelaxNG schématu (schéma je v souboru \codename{Bakalari.rng}).
Takový program zatím existuje jen pro Bakaláře\footnote{Tento program není součástí Vizevalu a je vyvíjen školou}.

Struktura je použitá z předchozí verze (více v \ref{uvod:predchozi-verze}), kořenový prvek se jmenuje \codename{BakalariImport} z historických důvodu. Emaily jsou v dobrovolné a pokud budou u některých studentů/učitelů chybět vygenerují se v dalším kroku importu.
...
To by však nemělo omezit kompatibilitu výrazů, jelikož RFC5321 specifikuje, že emailové schránky, které se využívají k přijímaní \unsure{goofy obrat} emailů by neměly mít lokální část ohraničenou uvozovkami\cite{rfc5321}.
...
Každá importovaná entita má atribut \codename{import} typu \codename{ImportStatus}.
